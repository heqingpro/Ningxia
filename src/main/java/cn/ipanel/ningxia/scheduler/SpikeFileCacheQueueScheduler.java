package cn.ipanel.ningxia.scheduler;
...
public class SpikeFileCacheQueueScheduler extends DuplicateRemovedScheduler implements MonitorableScheduler,Closeable {
        ...
        setDuplicateRemover(
                new DuplicateRemover() {
                    ...
                    public boolean isDuplicate(Request request, Task task) {
                        ...
                        boolean flag =  !urls.add(request.getUrl());
                		if(request.getUrl().matches(regx)){//二次校验，如果符合我们需要重新爬取的，返回false。可以重新爬取
                			flag = false;
                        }
                        return flag;
                    }
    ...
    protected void pushWhenNoDuplicate(Request request, Task task) {
        queue.add(request);
        //正则匹配,匹配到的不写入文件，减小文件大小
        if(request.getUrl().matches(regx)){//
        	return;
        }
